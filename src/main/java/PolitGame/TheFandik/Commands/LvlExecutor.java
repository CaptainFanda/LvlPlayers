...
import PolitGame.TheFandik.Service.UpgradeRewards;
...
import static PolitGame.TheFandik.Service.UpgradeRewards.giveReward;
import static PolitGame.TheFandik.Service.UpgradeRewards.removeRequiredItems;
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (sender instanceof Player p) {
            ...
            if (args.length == 1 && args[0].equals("set") || args.length == 1 && args[0].equals("check") || args.length == 1 && args[0].equals("give")) {
                ...
                int playerLevel = CF_LvlPlayers.getLevelFromPlayer(p);
                if (playerLevel <= ymlPutConfig.getInt("maxLvl")) {
                    List<String> paywall = ymlPutConfig.getStringList("paywall." + playerLevel);
                    if (UpgradeRewards.hasRequiredItems(p, paywall)) {
                        removeRequiredItems(p, paywall);
                        giveReward(p, playerLevel);
                        CF_LvlPlayers.giveLvlToPlayer(p, 1);
                        String message = prefix + (String) massages.get("succes-upgrade-lvl");
                        p.sendMessage(message);
                    } else {
                        String message = prefix + (String) massages.get("not-have-resourses");
                        p.sendMessage(message);
                    }
                } else {
                    String message = prefix + (String) massages.get("maxLvl");
                    p.sendMessage(message);
                }
            }
